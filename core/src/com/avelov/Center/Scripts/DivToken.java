...
public class DivToken implements Token {
    ...
    public DivToken(Token token_left, Token token_right)
    {
    ...
    public ExpressionPart GetExpressionPart()
    {
        if(token_left.isConst() == token_right.isConst())
        {
            if(token_left.isConst())
                return new DivExpressionPart(token_left.getValue(), token_right.getValue());
            else
                return new DivExpressionPart(token_left.GetExpressionPart(), token_right.GetExpressionPart());
        }
        else {
            if (token_left.isConst())
                return new DivExpressionPart(token_left.getValue(), token_right.GetExpressionPart());
            else
                return new DivExpressionPart(token_left.GetExpressionPart(), token_right.getValue());
        }
    }
}
