...
public class OrToken implements Token {
    ...
    public OrToken(Token token_left, Token token_right)
    {
    ...
    public float getValue() {
        ...
        System.err.println("Value of OrToken queried");
        return -1;
    }
    ...
    public ExpressionPart GetExpressionPart()
    {
        if(token_left.isConst() == token_right.isConst())
        {
            if(token_left.isConst())
                return new OrExpressionPart(token_left.getValue(), token_right.getValue());
            else
                return new OrExpressionPart(token_left.GetExpressionPart(), token_right.GetExpressionPart());
        }
        else {
            if (token_left.isConst())
                return new OrExpressionPart(token_left.getValue(), token_right.GetExpressionPart());
            else
                return new OrExpressionPart(token_left.GetExpressionPart(), token_right.getValue());
        }
    }
}
